"""
ai/services/embeddings/openai_provider.py – OpenAI-compatible embedding provider.
"""
    ...
    async def embed_documents(self, texts: list[str]) -> list[list[float]]:
        ...
        for attempt in range(1, _MAX_RETRIES + 1):
            try:
                ...
                logger.warning(f"OpenAI embedding attempt {attempt} failed: {exc}")
                last_err = exc
                if attempt < _MAX_RETRIES:
                    await asyncio.sleep(_RETRY_BASE_DELAY * attempt)

        raise RuntimeError(f"OpenAI embed_documents failed: {last_err}")
